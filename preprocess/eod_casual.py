...
class EOD_Preprocessor:
    ...
    def generate_feature(self, selected_tickers_fname, begin_date, opath,
        ...
        # print(index_tra_dates)
        # print(tra_dates_index)
        ...
        for stock_index, single_EOD in enumerate(self.data_EOD):
            ...
            for row in range(begin_date_row, selected_EOD.shape[0]):
                ...
                mov_aver_features1[row][0] = aver_5 / count_5
                mov_aver_features1[row][1] = aver_10 / count_10
                mov_aver_features1[row][2] = aver_20 / count_20
                mov_aver_features1[row][3] = aver_30 / count_30
            ...
            #                 (price_max - price_min)
            mov_aver_features1 = mov_aver_features1 / price_max

            mov_aver_features2 = np.zeros(
                [selected_EOD.shape[0], 4], dtype=float
            )   # 4 columns refers to 5-, 10-, 20-, 30-days average
            for row in range(begin_date_row, selected_EOD.shape[0]):
                date_index = selected_EOD[row][0]
                ...
                for offset in range(30):
                    ...
                    if date_gap < 5:
                        count_5 += 1
                        aver_5 += selected_EOD[row - offset][1]
                    if date_gap < 10:
                        count_10 += 1
                        aver_10 += selected_EOD[row - offset][1]
                    if date_gap < 20:
                        count_20 += 1
                        aver_20 += selected_EOD[row - offset][1]
                    if date_gap < 30:
                        count_30 += 1
                        aver_30 += selected_EOD[row - offset][1]
                mov_aver_features2[row][0] = aver_5 / count_5
                mov_aver_features2[row][1] = aver_10 / count_10
                mov_aver_features2[row][2] = aver_20 / count_20
                mov_aver_features2[row][3] = aver_30 / count_30
            ...
            '''
                ...
                return ratio calculation.
            '''
            pri_min = np.min(selected_EOD[begin_date_row:, 1])
            price_max = np.max(selected_EOD[begin_date_row:, 1])
            print(self.tickers[stock_index], 'minimum:', pri_min,
                  'maximum:', price_max, 'ratio:', price_max / pri_min)
            if price_max / pri_min > 10:
                print('!!!!!!!!!')
            # open_high_low = (selected_EOD[:, 1:4] - price_min) / \
            #                 (price_max - price_min)
            mov_aver_features2 = mov_aver_features2 / price_max

            mov_aver_features3 = np.zeros(
                [selected_EOD.shape[0], 4], dtype=float
            )   # 4 columns refers to 5-, 10-, 20-, 30-days average
            for row in range(begin_date_row, selected_EOD.shape[0]):
                date_index = selected_EOD[row][0]
                ...
                for offset in range(30):
                    ...
                    if date_gap < 5:
                        count_5 += 1
                        aver_5 += selected_EOD[row - offset][2]
                    if date_gap < 10:
                        count_10 += 1
                        aver_10 += selected_EOD[row - offset][2]
                    if date_gap < 20:
                        count_20 += 1
                        aver_20 += selected_EOD[row - offset][2]
                    if date_gap < 30:
                        count_30 += 1
                        aver_30 += selected_EOD[row - offset][2]
                mov_aver_features3[row][0] = aver_5 / count_5
                mov_aver_features3[row][1] = aver_10 / count_10
                mov_aver_features3[row][2] = aver_20 / count_20
                mov_aver_features3[row][3] = aver_30 / count_30
            ...
            '''
                ...
                return ratio calculation.
            '''
            pri_min = np.min(selected_EOD[begin_date_row:, 2])
            price_max = np.max(selected_EOD[begin_date_row:, 2])
            print(self.tickers[stock_index], 'minimum:', pri_min,
                  'maximum:', price_max, 'ratio:', price_max / pri_min)
            if price_max / pri_min > 10:
                print('!!!!!!!!!')
            # open_high_low = (selected_EOD[:, 1:4] - price_min) / \
            #                 (price_max - price_min)
            mov_aver_features3 = mov_aver_features3 / price_max


            mov_aver_features4 = np.zeros(
                [selected_EOD.shape[0], 4], dtype=float
            )   # 4 columns refers to 5-, 10-, 20-, 30-days average
            for row in range(begin_date_row, selected_EOD.shape[0]):
                date_index = selected_EOD[row][0]
                ...
                for offset in range(30):
                    ...
                    if date_gap < 5:
                        count_5 += 1
                        aver_5 += selected_EOD[row - offset][3]
                    if date_gap < 10:
                        count_10 += 1
                        aver_10 += selected_EOD[row - offset][3]
                    if date_gap < 20:
                        count_20 += 1
                        aver_20 += selected_EOD[row - offset][3]
                    if date_gap < 30:
                        count_30 += 1
                        aver_30 += selected_EOD[row - offset][3]
                mov_aver_features4[row][0] = aver_5 / count_5
                mov_aver_features4[row][1] = aver_10 / count_10
                mov_aver_features4[row][2] = aver_20 / count_20
                mov_aver_features4[row][3] = aver_30 / count_30
            ...
            '''
                ...
                return ratio calculation.
            '''
            pri_min = np.min(selected_EOD[begin_date_row:, 3])
            price_max = np.max(selected_EOD[begin_date_row:, 3])
            print(self.tickers[stock_index], 'minimum:', pri_min,
                  'maximum:', price_max, 'ratio:', price_max / pri_min)
            if price_max / pri_min > 10:
                print('!!!!!!!!!')
            # open_high_low = (selected_EOD[:, 1:4] - price_min) / \
            #                 (price_max - price_min)
            mov_aver_features4 = mov_aver_features4 / price_max
            ...
            #     [selected_EOD.shape[0], 16], dtype=float
            # )   # 4 columns refers to 5-, 10-, 20-, 30-days average
            mov_aver_features = np.concatenate([mov_aver_features1,mov_aver_features2,mov_aver_features3,mov_aver_features4],axis = 1)
            print(mov_aver_features)

            features = np.ones([len(trading_dates) - pad_begin, 18],
                               dtype=float) * -1234
            ...
            for row in range(begin_date_row, selected_EOD.shape[0]):
                ...
                features[cur_index - pad_begin][1:17] = mov_aver_features[
                    row]
