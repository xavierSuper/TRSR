...
def load_EOD_data(data_path, market_name, tickers, steps=1):
    print(data_path,market_name,tickers)
    eod_data = []
    masks = []
    ground_truth = []
    base_price = []
    for index, ticker in enumerate(tickers):
        single_EOD = np.genfromtxt(
            os.path.join(data_path, market_name + '_' + ticker + '_1.csv'),#开盘价、最高价、最低价、收盘价和交易量
    ...
    print(base_price)
    # 将字典保存为.pkl文件
    with open('eod_data.pkl', 'wb') as f:
        pickle.dump(eod_data, f)
    with open('mask_data.pkl', 'wb') as f:
        pickle.dump(masks, f)
    with open('gt_data.pkl', 'wb') as f:
        pickle.dump(ground_truth, f)
    with open('price.pkl', 'wb') as f:
        pickle.dump(base_price, f)
